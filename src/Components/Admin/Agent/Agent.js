...
const Agent = () => {
  ...
  const getAllServices = async () => {
    ...
    await axios
      .get(BASEURL + "/service-provider/all-services?page=1&limit=10", {
        headers,
      })
      .then((responce) => {
        if (responce) {
  ...
  const getAgentbyid = async (agentID) => {
    try {
      ...
      setLoading(true);
      await axios
        .get(`${BASEURL}/service-provider/agent/${agentID}`, { headers })
        .then((responce) => {
  ...
  const editAgent = async (e) => {
    ...
    await axios
      .put(`${BASEURL}/service-provider/agent/${id}`, data, { headers })
      .then((response) => {
        if (response) {
          window.history.back();
        }
