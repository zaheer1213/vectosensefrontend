...
const EditService = () => {
  ...
  const getAllCategory = async () => {
    ...
    try {
      const response = await fetch(
        `${BASEURL}/superadmin/service-category?page=1&limit=10`,
        {
          headers: {
  ...
  const handleSubmit = (event) => {
    ...
    if (id) {
      ...
      axios
        .put(`${BASEURL}/superadmin/service/${id}`, formData, {
          headers: {
            "x-access-token": token,
          },
        })
      ...
      axios
        .post(`${BASEURL}/superadmin/service`, formData, {
          headers: {
            "x-access-token": token,
          },
        })
  ...
  const getServiceById = async (serviceID) => {
    ...
    setLoading(true);
    await axios
      .get(`${BASEURL}/superadmin/service/${serviceID}`, { headers })
      .then((responce) => {
        setLoading(false);
