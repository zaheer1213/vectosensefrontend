...
import { BASEURL, COUNTRYURL } from "../../Commanconstans/Comman";
...
const EditBusiness = () => {
  ...
  const handleSubmit = async (e) => {
    if (business_id) {
      ...
      await axios
        .put(`${BASEURL}/superadmin/business/${business_id}`, data, {
          headers,
        })
        .then((response) => {
          if (response.data) {
  ...
  const getBusinessDetilsByID = async (businessId) => {
    ...
    setLoading(true);
    await axios
      .get(`${BASEURL}/superadmin/business/${businessId}`, { headers })
      .then((responce) => {
